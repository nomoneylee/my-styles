...
document.addEventListener('DOMContentLoaded', () => {
  // 0. Navbar 行動端漢堡選單對折
  const navToggler = document.getElementById('swNavbarToggler');
  const navCollapse = document.getElementById('swNavbarCollapse');
  ...
  const accordionHeaders = document.querySelectorAll('.sw-accordion-header');
  ...
  // 2. Modal 對話框控制
  const modalOverlay = document.getElementById('swModalOverlay');
  const openModalBtn = document.getElementById('openModal');
  const closeModalBtns = document.querySelectorAll('.sw-close-modal');
  ...
  // 3. Offcanvas 側邊欄
  const offcanvas = document.getElementById('swOffcanvas');
