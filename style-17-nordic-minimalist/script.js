...
document.addEventListener('DOMContentLoaded', () => {
  // 0. Navbar 行動端漢堡選單對折
  const navToggler = document.getElementById('ndNavbarToggler');
  const navCollapse = document.getElementById('ndNavbarCollapse');
  ...
  const accordionHeaders = document.querySelectorAll('.nd-accordion-header');
  ...
  // 2. Modal 對話框控制
  const modalOverlay = document.getElementById('ndModalOverlay');
  const openModalBtn = document.getElementById('openModal');
  const closeModalBtns = document.querySelectorAll('.nd-close-modal');
  ...
  // 4. Tabs 分頁切換
  const tabBtns = document.querySelectorAll('.nd-tab-btn');
