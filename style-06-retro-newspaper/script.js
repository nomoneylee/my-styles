/**
 * Style 06: Retro Newspaper & Vintage Magazine (Pure Vanilla JS)
 */
document.addEventListener('DOMContentLoaded', () => {
  // 0. Navbar 行動端漢堡選單對折 Collapse
  const toggler = document.getElementById('npNavbarToggler');
  const collapse = document.getElementById('npNavbarCollapse');
  if (toggler && collapse) {
    toggler.addEventListener('click', () => collapse.classList.toggle('show'));
  }

  // 1. Accordion 手風琴折疊
  document.querySelectorAll('.np-accordion-header').forEach(header => {
