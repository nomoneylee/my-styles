/**
 * Style 10: Streetwear Brand / Music Festival Poster (Pure Vanilla JS)
 */
document.addEventListener('DOMContentLoaded', () => {
  // 0. Navbar 行動端漢堡選單對折 Collapse
  const toggler = document.getElementById('stNavbarToggler');
  const collapse = document.getElementById('stNavbarCollapse');
  if (toggler && collapse) {
    toggler.addEventListener('click', () => collapse.classList.toggle('show'));
  }

  // 1. Accordion 手風琴折疊
  document.querySelectorAll('.st-accordion-header').forEach(header => {
