...
document.addEventListener('DOMContentLoaded', () => {
  // 0. Navbar 行動端漢堡選單對折
  const navToggler = document.getElementById('krNavbarToggler');
  const navCollapse = document.getElementById('krNavbarCollapse');
  ...
  // 2. Modal 對話框控制
  const modalOverlay = document.getElementById('krModalOverlay');
  const openModalBtn = document.getElementById('openModal');
  const closeModalBtns = document.querySelectorAll('.kr-close-modal');
  ...
  // 3. Offcanvas 側邊欄
  const offcanvas = document.getElementById('krOffcanvas');
  ...
  // 4. Tabs 分頁切換
  const tabBtns = document.querySelectorAll('.kr-tab-btn');
  const tabPanes = document.querySelectorAll('.kr-tab-pane');
