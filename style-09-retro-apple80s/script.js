/**
 * Style 09: 80s Apple Computer / Classic Vintage Electronic (Pure Vanilla JS)
 */
document.addEventListener('DOMContentLoaded', () => {
  // 0. Navbar 行動端漢堡選單對折 Collapse
  const toggler = document.getElementById('mcNavbarToggler');
  const collapse = document.getElementById('mcNavbarCollapse');
  if (toggler && collapse) {
    toggler.addEventListener('click', () => collapse.classList.toggle('show'));
  }

  // 1. Accordion 手風琴折疊
  document.querySelectorAll('.mc-accordion-header').forEach(header => {
