/**
 * Style 03: Outdoor Camping & Cafe Menu (Pure Vanilla JS)
 */
document.addEventListener('DOMContentLoaded', () => {
  // 0. Navbar 行動端漢堡選單對折 Collapse
  const toggler = document.getElementById('cwNavbarToggler');
  const collapse = document.getElementById('cwNavbarCollapse');
  if (toggler && collapse) {
    toggler.addEventListener('click', () => collapse.classList.toggle('show'));
  }

  // 1. Accordion 手風琴折疊
  document.querySelectorAll('.cw-accordion-header').forEach(header => {
